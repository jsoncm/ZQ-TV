// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["tyyszy", "zy360","wolong", "ruyi", "xinlang"]'); // 默认选中天涯资源、暴风资源和如意资源
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 仅选择天涯资源、如意资源
        selectedAPIs = ["tyyszy", "zy360","wolong", "ruyi", "xinlang"];
...
function checkAdultAPIsSelected() {
    ...
    if (hasAdultSelected) {
        ...
        // 恢复原来的描述文字
        if (filterDescription) {
            filterDescription.innerHTML = '过滤黄色内容';
        }
...
async function importConfig() {
    showImportBox(async (file) => {
        try {
            ...
            if (config.name !== 'ZQ-TV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'ZQ-TV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'ZQ-TV-Settings_' + times + '.json');
}
